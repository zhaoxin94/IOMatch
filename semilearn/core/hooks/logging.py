...
class LoggingHook(Hook):
    ...
    def after_train_step(self, algorithm):
        ...
        if self.every_n_iters(algorithm, algorithm.num_eval_iter):
            if not algorithm.distributed or (algorithm.distributed
                ...
                algorithm.print_fn(
                    f"Open-Set Evaluation: Acc:{algorithm.eval_dict['o_acc'] * 100:.2f}, Precision:{algorithm.eval_dict['o_precision'] * 100:.2f}, RECALL:{algorithm.eval_dict['o_recall'] * 100:.2f}, F1-score:{algorithm.eval_dict['o_f1'] * 100:.2f}, AUROC:{algorithm.eval_dict['o_auroc']*100:.2f},H-score:{algorithm.eval_dict['o_hscore']*100:.2f}, Known-acc:{algorithm.eval_dict['o_knownacc']*100:.2f}, Unknown-acc:{algorithm.eval_dict['o_unknownacc']*100:.2f}"
                )
                algorithm.print_fn(
    ...
    def after_run(self, algorithm):
        ...
        algorithm.print_fn(
                    f"Open-Set Evaluation: Acc:{best_results['o_acc'] * 100:.2f}, Precision:{best_results['o_precision'] * 100:.2f}, RECALL:{best_results['o_recall'] * 100:.2f}, F1-score:{best_results['o_f1'] * 100:.2f}, AUROC:{best_results['o_auroc']*100:.2f},H-score:{best_results['o_hscore']*100:.2f}, Known-acc:{best_results['o_knownacc']*100:.2f}, Unknown-acc:{best_results['o_unknownacc']*100:.2f}"
                )
