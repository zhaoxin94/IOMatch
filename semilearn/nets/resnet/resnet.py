...
class ResNet50(nn.Module):
    def __init__(
        ...
        self.old_fc = nn.Linear(512 * block.expansion, num_classes)
    ...
    def group_matcher(self, coarse=False, prefix=''):
        matcher = dict(stem=r'^{}conv1|^{}bn1|^{}maxpool'.format(
            prefix, prefix, prefix),
                       blocks=r'^{}layer(\d+)'.format(prefix)
                       if coarse else r'^{}layer(\d+)\.(\d+)'.format(prefix))
        return matcher
...
def init_pretrained_weights(model, model_url):
    pretrain_dict = load_url(model_url)
